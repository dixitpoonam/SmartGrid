...
function delete1(v,s,t) {
  ...
  var txt="DO YOU WANT TO DELETE THIS IMAGE ?? Upload_time = "+v;
  ...
  var status="Delete";
    ...
    if (confirm(txt)) {
    	...
    	location.assign("mediacenter.jsp?Upload_time="+v+"&Media_Name="+s+"&status="+status+"&Media_Type="+t);
...
function Redirect()
{
	alert("Record Delete Successfull..!"+v);
...
<table id="example" class="display" style="width:100%" cellspacing="0" >
 <thead>
	...
	<%
	     ...
	     System.out.println("id -----"+id); 
			...
			while (result.next()) {
				...
				Upload_time = result.getString("Datetime"); //
				Media_Name = result.getString("originalmedianame"); //
				Media_Type = result.getString("mediatype");   //
				Duration = result.getString("duration");
				Desc = result.getString("describtion");
	...
	%>
	               ...
	               <tr align="center">
						<td  align="center">
						<a href="#" title="Click Here for Delete Media" onclick="return delete1('<%=Upload_time %>','<%=Media_Name%>','<%=Media_Type%>')">
			            <img src="images/delete.jpeg" alt="Delete Record" width="20px" height="18px" style="border-style: none"></img></a></td>
						<td align="right"><%=i%></td>
						<td align="right"><%=Upload_time%></td>
						<td align="left"><a href="http://twtech.in/SmartGrid/Media/<%=id%>"><%=Media_Name%></a></td>
    	...
    	int count;
    	  Upload_time=request.getParameter("Upload_time");
    	String detetequery="delete from smartgrid.t_globalmediamaster where changemedianame='"+Upload_time+"'";
