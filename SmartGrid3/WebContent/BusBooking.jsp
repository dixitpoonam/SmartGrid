...
//     	  alert(state);
    	  ...
    	  var area=document.getElementById("city").value;
 		  ...
 		  return false;
              ...
              window.location.replace("BusBooking.jsp?state="+state+"&city="+city+"&area="+area+"&chk=Bus");
   }
      function addarea()
      {
...
// 				alert("reslt"+reslt);
		...
		var queryString ="?lat="+lat+"&lon="+lon+"&enduser=volkswagen";
		//alert(queryString);
		ajaxRequest.open("POST", "ajaxenduser.jsp" + queryString, true);
		ajaxRequest.send(null); 
...
// 				alert("reslt"+reslt);
		...
		var queryString ="?lat="+lat+"&lon="+lon+"&enduser=Mercedes";
		//alert(queryString);
		ajaxRequest.open("POST", "ajaxenduser.jsp" + queryString, true);
		ajaxRequest.send(null); 
   ...
   <tr>
    ...
    <div class="styled-select slate">
				<select  id="country1" name="country1"  onchange="showbox();showbox2();"    >
				  	...
				  	{
				  	%>
				  		<option value="Select"><%=request.getParameter("state") %></option>
	...
	<div class="styled-select slate">
				<select  disabled  id="state"  name="state"   onchange="showbox1()" > 
					<%
				  	...
				  	{
				  	%>
				  		<option value="Select"><%=request.getParameter("city") %></option>
	...
	<div class="styled-select slate">
				<select  disabled  id="city" name="city"  style='width: px; height: auto;'>
					<%
				  	...
				  	{
				  	%>
				  		<option value="Select"><%=request.getParameter("area") %></option>
...
<div class="absolute1" align="center" >
<div class="absolute1" align="left">
						<% 
						int j=1;
						String EndCustomer="";
						 String Sqlday="";
	...
	%>
	<%
String distance="SELECT Distinct(c.End_Customer) FROM smartgrid.t_routepoints a inner join smartgrid.t_routemaster b on a.Routeid=b.Routeid inner join smartgrid.t_polmaster c on  c.vehiclecode=b.vehiclecode where  ((2.0 * 3958.75 * ATAN(SQRT(SIN(RADIANS(a.Lattiude - "+ lat +" )*0.5) * SIN(RADIANS(a.Lattiude - "+ lat +")*0.5) + COS(RADIANS(18.471)) * COS(RADIANS(a.Lattiude)) *SIN(RADIANS(a.Longitude - "+ lon +")*0.5) *SIN(RADIANS(a.Longitude - "+ lon +")*0.5)), SQRT(1.0-(SIN(RADIANS(a.Lattiude - 18.471)*0.5) *SIN(RADIANS(a.Lattiude - "+ lat +")*0.5) +COS(RADIANS(19.4672)) * COS(RADIANS(a.Lattiude)) *SIN(RADIANS(a.Longitude - "+ lon +")*0.5) *SIN(RADIANS(a.Longitude - "+ lon +")*0.5)) )) * 1609.344)/1000) <=2.5  and c.AssetType='Bus' ;";
...
while(rst.next())
{
	EndCustomer=rst.getString("End_Customer");
// 	out.println("End:-"+EndCustomer);
%>
 	<br>
<br>	
<%
if(EndCustomer.equalsIgnoreCase("Mercedes"))
{
%>
	 
<div align="left"><a onMouseover="change();"><i class="material-icons" style="font-size:48px;color:blue">edit_location</i></a><%= EndCustomer%></div>

<%
}

else if(EndCustomer.equalsIgnoreCase("volkswagen"))
{
%>

<div align="left"><a onMouseover="change1();"><i class="material-icons" style="font-size:48px;color:yellow">edit_location</i></a><%= EndCustomer%></div>
...
<%--  	<input type="hidden" name="cntr" id="cntr"  value="<%=i%>"> --%>
 ...
 <br> 
 <br>     
<div class="absolute1" align="center" id="thisdaydiv" >
</div>
